...
class DeliveryPage extends JFrame implements ActionListener {
   ...
   public void loadDeliveryTable() {
		...
		try {
			while (rs.next()) {
				delivery.insertRow(i, new Object[]{rs.getString(1), rs.getString(2), rs.getString(3), rs.getString(4), rs.getString(5), rs.getString(6),rs.getString(7), rs.getString(8)});
				i++;
			}
		} catch (Exception e) {
			e.printStackTrace();
